...
ax3.scatter(df['time_to_the_metro'], df['rent_price_per_metr'], alpha=0.5, color='green', edgecolor='black')
...
df['trash_chute'].value_counts().plot(kind='pie', autopct='%1.1f%%', ax=ax6, startangle=90)
ax6.set_ylabel('')
ax6.set_title('Наличие мусоропровода')
st.pyplot(fig6)

# Группировка
df_metro_avg = df.groupby('metro')['price'].median().sort_values(ascending=False).head(10).reset_index()
